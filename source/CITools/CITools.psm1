        ...
        while ($components.Count -gt 0)
        {
            ...
            $Steps[$stepName] | ForEach-Object {
                ...
                $entry | ForEach-Object {
                    ...
                    # If it's a script block, generate a placeholder name to use and
                    # map that placeholder to the script
                    if ($dependency.GetType().FullName -eq "System.Management.Automation.ScriptBlock")
                    {
                        $script = $dependency
                        $dependency = ("{0}_{1}" -f $stepName, [Guid]::NewGuid().ToString())
                    ...
                    # Make sure we are working with a string at this point
                    if ($dependency.GetType().FullName -ne "System.String")
                    {
                        Write-Error ("Invalid type in step definition: " + $dependency.GetType().FullName)
                    }

                    # Define a dependency for the parent to this item
                    Add-CIStepDependency -DependencyMap $dependencyMap -StepName $stepName -Dependency $dependency

                    # Define a dependency for this item to the previous item, if there is one
                    if ($null -ne $previousDependency)
                    {
